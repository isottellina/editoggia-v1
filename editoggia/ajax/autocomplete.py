...
def autocomplete(model_name):
    ...
    if model_name == "Fandom":
        count_stmt = (
            db.session.query(
                FandomStories.fandom_id, func.count("*").label("story_count")
            )
            .group_by(FandomStories.fandom_id)
            .subquery()
        )
        results = (
            db.session.query(Fandom.name)
        ...
        count_stmt = (
            db.session.query(StoryTags.tag_id, func.count("*").label("story_count"))
            .group_by(StoryTags.tag_id)
            .subquery()
        )
        results = (
