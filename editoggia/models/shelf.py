...
class Shelf(db.Model, CRUDMixin):
    ...
    stories = db.relationship('Story', secondary='shelves_stories', back_populates='shelves')
...
class ShelvesStories(db.Model):
    ...
    shelf_id = db.Column('shelf_id', db.Integer(), db.ForeignKey('shelf.id'), nullable=False)
