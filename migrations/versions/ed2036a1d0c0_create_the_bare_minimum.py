...
from editoggia.models import Permission, Role, Fandom, FandomCategory
...
roles = [
    ("Administrator", "Administrator of the website.", ("admin.ACCESS_ADMIN_INTERFACE", "mod.ACCESS_TAG_INTERFACE", )),
...
def upgrade():
    ...
    for category in ["Anime", "Books", "Cartoons", "Movies", "Video Games", "TV Shows", "Other"]:
        op.execute(
            sa.insert(FandomCategory).values(
                name=category
            )
        )
    ...
    for role in roles:
        ...
        role_id = bind.scalar(
            sa.insert(Role).values(name=name, description=description).returning(Role.id)
        )

        for permission_name in role_permissions:
            permission_id = bind.scalar(sa.select(Permission).where(Permission.name==permission_name))
            op.execute(sa.insert(PermissionsRoles).values(perm_id=permission_id, role_id=role_id))
